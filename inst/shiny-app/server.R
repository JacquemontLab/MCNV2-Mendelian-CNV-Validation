...
function(input, output, session) {
  ...
  observeEvent(input$submit_ft_mpviz, {
    withProgress(message = "Running analysis...", value = 0, {
      ...
      output$after_add_filters <- renderPlotly({
        ...
        # Aucun chamgement
        n1 <- filtered_ds() %>% dplyr::count() %>% dplyr::collect() %>% dplyr::pull(n)
        n2 <- ft_filtered_ds() %>% dplyr::count() %>% dplyr::collect() %>% dplyr::pull(n)
  ...
  observeEvent(c(input$comp_plot1_type, input$submit_ft_mpviz), {
    ...
    output$comp_plot1 <- renderPlotly({
      ...
      # Aucun chamgement
      n_df1 <- df1 %>% dplyr::count() %>% dplyr::collect() %>% dplyr::pull(n)
      if (is.null(df1) || n_df1 == 0) {
